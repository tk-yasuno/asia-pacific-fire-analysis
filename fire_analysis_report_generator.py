...
class FireAnalysisReportGenerator:
    ...
    def _generate_executive_summary(self, clustering_result, feature_analysis, nasa_df, config) -> str:
        ...
        return f"""# インド太平洋地域火災検知分析レポート
## Executive Summary - エグゼクティブサマリー

**分析期間**: 過去{config['nasa_firms']['days_back']}日間 ({datetime.now().strftime('%Y年%m月%d日')}時点)  
**対象地域**: インド太平洋地域 ({area_params['west']}°E - {area_params['east']}°E, {area_params['south']}°S - {area_params['north']}°N)  
...
- **品質スコア**: {clustering_result.metrics.quality_score:.3f} (高品質クラスタリング達成)
...
| ノイズ率 | {clustering_result.metrics.noise_ratio:.1%} |
...
#### 全体パフォーマンス
- **選択手法**: {clustering_result.method}
- **品質スコア**: {clustering_result.metrics.quality_score:.3f}/1.0
- **シルエット係数**: {clustering_result.metrics.silhouette_score:.3f}
